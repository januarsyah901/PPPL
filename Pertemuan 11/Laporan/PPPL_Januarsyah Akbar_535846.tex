...
Sebuah kelas bernama \texttt{RunCucumberTest.java} dibuat pada paket \texttt{runner} (\texttt{src/test/java/runner}). Berbeda dengan JUnit 4 tradisional yang memakai \texttt{@RunWith(Cucumber.class)}, implementasi ini menggunakan \textbf{JUnit Platform Suite Engine} (JUnit 5) agar dapat mengonfigurasi parameter filter tag secara deklaratif via anotasi \texttt{@ConfigurationParameter} untuk menyaring skenario pengujian yang akan dijalankan.

\begin{lstlisting}[style=javastyle, caption={Implementasi Kelas RunCucumberTest.java dengan JUnit Platform Suite}]
package runner;
...
Kelas pemetaan langkah fungsional pencarian Bing diimplementasikan di \texttt{src/test/java/stepDef/SearchSteps.java} dengan mengacu pada inisialisasi \texttt{WebDriver} dari Cucumber Hooks:
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.85\textwidth]{screenshot step/Latihan_test_result.png}}
...
Kelas implementasi langkah fungsional untuk autentikasi login SauceDemo ditulis pada \texttt{src/test/java/stepDef/LoginSteps.java} dengan mengacu pada inisialisasi \texttt{WebDriver} dari Cucumber Hooks:
...
\subsection{Implementasi Cucumber Hooks}
Untuk mengotomatisasi daur hidup \texttt{WebDriver} (\textit{browser creation and teardown}), diimplementasikan modul \textbf{Cucumber Hooks} (\texttt{@Before} dan \texttt{@After}). Daur hidup diatur secara global dalam satu berkas terpisah bernama \texttt{Hooks.java} di dalam paket \texttt{stepDef}:

\begin{lstlisting}[style=javastyle, caption={Implementasi Kelas Hooks.java}]
package stepDef;
...
Dengan diimplementasikannya berkas \texttt{Hooks.java}, pemanggilan inisialisasi \texttt{new ChromeDriver()} dan penutupan browser \texttt{driver.quit()} pada masing-masing langkah pengujian fungsional (\texttt{LoginSteps.java} dan \texttt{SearchSteps.java}) dapat dieliminasi secara utuh. Hal ini mencegah terjadinya redudansi kode serta kebocoran memori (\textit{browser ghost processes}) jika ada asersi uji yang gagal di tengah proses.
\clearpage
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.85\textwidth]{screenshot step/Tugas_test_result_success.png}}
...
\begin{figure}[H]
    \centering
    \customfbox{\includegraphics[width=0.85\textwidth]{screenshot step/Tugas_test_result_failure.png}}
...
\section{Repositori Kode Sumber (Resource)}
Dapatkan full code pada link github di bawah ini:
\begin{center}
    \url{https://github.com/januarsyah901/PPPL/tree/5a3220e3abf508bef4469c541536b9ecb77a9eed/Pertemuan%2011}
\end{center}
